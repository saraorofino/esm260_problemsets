...
crab_1 <- ggplot(crab1, aes(x = crab_ca_ad, y = crab_ca_rec)) +
  geom_point() +
...
crab_2 <- ggplot(crab1, aes(x = crab_or_ad, y = crab_or_rec)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y~x, se=FALSE, size = 0.5) + 
  theme_light()
crab_2
...
crab_3 <- ggplot(crab1, aes(x = crab_ca_ad, y = crab_or_rec)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y~x, se=FALSE, size=0.5) +
  theme_light()
crab_3
...
#Recruits (2001) vs. winter upwelling in CA and OR (2001)
b_winter_ca_rec2 <- ggplot(crab2, aes(x = barnacle_ca_rec, y = winter_ca)) + 
  geom_point() + 
  geom_smooth(method = "lm", formula = y~x, se=FALSE, size=0.5) + 
  theme_bw()
b_winter_ca_rec2
...
b_winter_or_rec2 <- ggplot(crab2, aes(x = barnacle_or_rec, y = winter_or)) + 
  geom_point() + 
  geom_smooth(method = "lm", formula = y~x, se=FALSE, size=0.5) + 
  theme_bw()
b_winter_or_rec2
 ...
 * CA crab recruits vs. winter upwelling
 * OR barnacle recruits vs. summer upwelling  
 * CA adult barnacles vs. winter upwelling  
 * CA barnacle recruits (2001) vs. winter upwelling (2001)
 
Try a regression for these relationships to see if they are significant:  
```{r}
#adult crab density vs ca recruits
or_ca_ad <- lm(formula = crab_ca_rec ~ crab_or_ad, data = crab1)
summary(or_ca_ad) #not a significant relationship
...
#ca recruits (year x) vs. winter upwelling (year x)
ca_rec_up <- lm(formula = crab_ca_rec ~ winter_ca, data=crab2)
summary(ca_rec_up) #significant at 0.031
...
#ca recruits (year x) vs. winter upwelling (year x)
ca_b_rec_up <- lm(formula = barnacle_ca_rec ~ winter_ca, data=crab2)
summary(ca_b_rec_up) #very significant < 0.0001
...
#CA crab recruits (year x) vs. winter upwelling in CA (year x):
winter_ca_rec2 <- ggplot(crab2, aes(x = winter_ca, y = crab_ca_rec)) + 
  geom_point() + 
  geom_smooth(method = "lm", formula = y~x, se=FALSE, size=0.5) + 
  theme_bw()
winter_ca_rec2
...
#CA barnacle recruits (year x) vs. winter upwelling in CA (year x)
